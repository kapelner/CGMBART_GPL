...
import CGM_BART.TreeArrayIllustration;
...
public abstract class CGMPosteriorBuilder {
	...
	public CGMTreeNode convergePosteriorAndFindMostLikelyTree(CGMTreeNode T_0, int max_num_iterations, int num_restart) {
		...
		if (DEBUG_ITERATIONS){
			...
			iteration_info.put("num_restart", TreeArrayIllustration.LeadingZeroes(num_restart, 3));
//			new TreeIllustration(T_0, iteration_info).WriteTitleAndSaveImage();
		...
		while (true){	
			...
			iteration_info = new HashMap<String, String>();
			iteration_info.put("num_restart", TreeArrayIllustration.LeadingZeroes(num_restart, 3)); 
			iteration_info.put("num_iteration", TreeArrayIllustration.LeadingZeroes(num_iterations, 5));
	...
	protected void createTreeProposalViaChange(CGMTreeNode T) {
		...
		if (DEBUG_ITERATIONS){
			...
			CGMBART.mh_iterations_full_record.print(
				...
				TreeArrayIllustration.LeadingZeroes(Double.parseDouble(TreeIllustration.one_digit_format.format(presplitValue)), 4) + "," +					
				"X_" + (internal_node_to_change.splitAttributeM + 1) + "," + 
				TreeArrayIllustration.LeadingZeroes(Double.parseDouble(TreeIllustration.one_digit_format.format(internal_node_to_change.splitValue)), 4) + ","		
			);
		}				
