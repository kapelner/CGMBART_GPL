...
//		System.out.println("node:" + node.stringID() + " leaf:" + node.isLeaf + " left: " + node.left + " right:" + node.right);
		...
		if (node.isLeaf && node.y_pred != CGMBARTTreeNode.BAD_FLAG_double){
		...
		else if (node.splitAttributeM != CGMBARTTreeNode.BAD_FLAG_int && node.splitValue != CGMBARTTreeNode.BAD_FLAG_double) {
