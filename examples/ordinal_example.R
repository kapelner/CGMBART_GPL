...
library(MASS)
data(Boston)
X = Boston[order(Boston$age), ]
#X = cbind(X, rnorm(nrow(X)))
y = X$medv
y = as.integer(ifelse(y < 10, 10, ifelse(y < 20, 20, ifelse(y < 30, 30, ifelse(y < 40, 40, 50)))))
X$medv = NULL
...
bart_machines = build_ordinal_bart_machine(Xtrain, ytrain,
		num_trees = 200,
		num_burn_in = 300,
