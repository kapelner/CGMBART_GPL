...
source("r_scripts/bart_validation.R")
source("r_scripts/create_simulated_models.R")
graphics.off()

source("r_scripts/bakeoff/bart_bakeoff_params.R")
...
run_bart_bakeoff_iter_num = function(iter_num){
	...
	#save results
	write.csv(results, paste("bart_bakeoff_results_", iter_num, ".csv", sep = ""))
}
...
run_models_and_save_results = function(training_data, test_data, model){
	...
	#############
	time_started = Sys.time()
	bart_machine_cv = bart_machine_cv(Xtrain, ytrain, 
			num_burn_in = num_burn_in, 
			num_iterations_after_burn_in = num_iterations_after_burn_in)
