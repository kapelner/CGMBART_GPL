...
%&=& \underbrace{\oneover{\tothepow{2\pi\sigsq}{n_\ell / 2}} ~\exp{-\oneover{2\sigsq} \sum_{i=1}^{n_\ell} R_{\ell_i}^2}}_h \underbrace{\exp{-\oneover{2\sigsq} \parens{-2n_\ell \mu \Rbar_\ell + n_\ell \musq}}}_g \\
...
%\end{changemargin}
%
%Note how the overbraces are only functions of $\sum R_i$ which makes calculations efficient.
